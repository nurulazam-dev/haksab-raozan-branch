...
const NoticeList = () => {
    ...
    return (
        <div className="p-4">
            ...
            <Link to="/notices/add" className="mb-4 inline-block bg-blue-500 text-white px-4 py-2 rounded">
                Add Notice
            </Link>
            <table className="min-w-full bg-white border border-gray-300">
                ...
                <tbody>
                    {notices.map((notice) => (
                        <tr key={notice.id}>
                            <td className="border-b border-gray-300 px-4 py-2">{notice.title}</td>
                            <td className="border-b border-gray-300 px-4 py-2">{new Date(notice.date).toLocaleDateString()}</td>
                            <td className="border-b border-gray-300 px-4 py-2">
                                <Link to={`/notices/edit/${notice.id}`} className="text-blue-500">Edit</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
